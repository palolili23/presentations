...
background-image: url(./figs/inverse_risk_factors.PNG)
background-size: 100%
...
.pull-left[
- Estimating effects of modifiable risk factors on dementia risk relies on observational studies studies

- Observational studies often focus on single time-point exposure assessment rather than the longitudinal effects

- We need causal methods to guide prevention strategies
]

.pull-right[
<img src=./figs/lancet_paf.png width="70%"/> ]
...
target_ht <-
  tibble::tribble(
    ...
    "Same",
    "Causal contrast",
    "What would have been the risk of dementia if all individuals adhered to their assigned strategy and death was eliminated (Controlled direct per protocol effect)",
...
target_ht %>% select(-n) %>% gt() %>% 
    ...
    "Emulation using observational data" ~ px(320)) %>% 
  tab_options(
        table.font.size = 22) %>% 
  tab_style(
            style = cell_text(align = "center", weight = "bold"),
